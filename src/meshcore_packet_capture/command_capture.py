...
async def process_mqtt_command(
    ...
    if command == "device_query":
        await _run_command("device_query", lambda: capture.meshcore.commands.send_device_query())
        return

    if command == "get_battery":
        await _run_command("get_battery", lambda: capture.meshcore.commands.get_bat())
        return

    if command == "set_name":
        name = payload_data.get("name")
        if not name or not isinstance(name, str):
            capture.logger.warning("set_name requires string 'name'")
            return
        await _run_command("set_name", lambda: capture.meshcore.commands.set_name(name))
        return

    if command == "send_advert":
        flood = bool(payload_data.get("flood", False))
        await _run_command("send_advert", lambda: capture.meshcore.commands.send_advert(flood=flood))
        return
    ...
    if command == "send_trace":
        ...
        await _run_command("send_trace", lambda: capture.meshcore.commands.send_trace(**kwargs))
        return

    if command == "send_telemetry_req":
        destination = payload_data.get("destination")
        if not destination or not isinstance(destination, str):
            capture.logger.warning("send_telemetry_req requires string 'destination'")
            return

        password = payload_data.get("password")
        if password:
            if not isinstance(password, str):
                capture.logger.warning("send_telemetry_req 'password' must be string when provided")
                return
            await _run_command(
                "send_login(auto)",
                lambda: capture.meshcore.commands.send_login(destination, password),
            )

        await _run_command(
            "send_telemetry_req",
            lambda: capture.meshcore.commands.send_telemetry_req(destination),
        )
        return
    ...
    if command == "send_login":
        ...
        password = payload_data.get("password")
        if not destination or not isinstance(destination, str):
            capture.logger.warning("send_login requires string 'destination'")
            return
        if not password or not isinstance(password, str):
            capture.logger.warning("send_login requires string 'password'")
            return
        await _run_command(
            "send_login",
            lambda: capture.meshcore.commands.send_login(destination, password),
    ...
    if command == "send_logoff":
        destination = payload_data.get("destination")
        if not destination or not isinstance(destination, str):
            capture.logger.warning("send_logoff requires string 'destination'")
            return
        await _run_command(
            "send_logoff",
            lambda: capture.meshcore.commands.send_logoff(destination),
        )
        return
