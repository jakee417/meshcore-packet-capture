...
async def process_mqtt_command(
    ...
    async def _run_command(
        ...
        if on_success is not None:
            ...
            capture.logger.info(f"MQTT command '{_sanitize_log_field(command_name)}' succeeded on {broker_label}")
        return True
    ...
    capture.logger.warning(f"Unknown MQTT command '{_sanitize_log_field(command)}' from {broker_label}")
